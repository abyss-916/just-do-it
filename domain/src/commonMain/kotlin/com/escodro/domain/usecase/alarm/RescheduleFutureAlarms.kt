...
class RescheduleFutureAlarms(
    ...
    suspend operator fun invoke() {
        val uncompletedAlarms = taskRepository.findAllTasksWithAlarmDate().filterNot { it.isCompleted }
        val futureAlarms = uncompletedAlarms.filter { isInFuture(it.alarmDate) }
    ...
    private fun isMissedRepeating(task: Task): Boolean {
        val currentTime = dateTimeProvider.getCurrentInstant()
        val givenTime = task.alarmDate?.toInstant(TimeZone.currentSystemDefault()) ?: return false
        return task.isRepeating && currentTime > givenTime
    }

    private fun rescheduleFutureTask(task: Task) {
        val futureTime = task.alarmDate
            ?.run {
        ...
        logger.debug { "Task '${task.title}' rescheduled to '${task.alarmDate}'" }
    }

    @Suppress("NullableToStringCall")
    private suspend fun rescheduleRepeatingTask(task: Task) {
        scheduleNextAlarm(task)
        logger.debug { "Repeating task '${task.title}' rescheduled to '${task.alarmDate}'" }
    }
}
